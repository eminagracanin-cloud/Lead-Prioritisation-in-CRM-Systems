...
.hero {
    ...
    color: white;
    margin-bottom: 28px;
    box-shadow: 0px 14px 35px rgba(15, 23, 42, 0.22);
}
...
.dark-card {
    ...
    border-radius: 20px;
    color: white;
    box-shadow: 0px 8px 24px rgba(15, 23, 42, 0.16);
    margin-bottom: 18px;
    min-height: 185px;
}
...
def gemini_final_recommendation(priority, source, probability, contact_status, recommendation_category):
...
Write one short final recommendation for a marketing or sales team.
Keep it simple, practical, and business-oriented.
...
def build_export_data(data):
    ...
    export_df["RecommendationCategory"] = export_df.apply(
        lambda row: assign_recommendation_category(row["Priority"], row["ContactStatus"]),
        axis=1
    )

    export_df["AIRecommendation"] = export_df["RecommendationCategory"].apply(generate_recommendation)
...
if page == "Lead Evaluation":
    ...
    if st.button("🚀 Evaluate Lead", use_container_width=True):
        ...
        recommendation = gemini_final_recommendation(
            priority,
        ...
        human_review = human_review_status(priority, contact_status)
...
# =========================================================
    ...
    st.caption(
        "Filter leads by priority, contact status, recommendation category, and lead source. "
        "Download the selected leads for CRM follow-up."
    )
...
# =========================================================
    ...
    if os.path.exists("logs.json") and os.path.getsize("logs.json") > 0:
        ...
        st.info("No predictions logged yet.")    padding-top: 2rem;
    padding-bottom: 2rem;
}
